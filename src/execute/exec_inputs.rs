use super::{child::handle_child, job_log::JobLog, pipe::disk::State};
...
impl<IO: Read> ExecInputs<IO> {
    pub fn run(&mut self, mut flags: u16) {
        ...
        while let Some(job_id) = self.inputs.try_next(&mut input) {
            ...
            let (start_time, end_time, exit_value, signal) =
                match command::get_command_output(&input, flags) {
                    Ok(child) => handle_child(
                        child,
                        &self.output_tx,
                        flags,
                        job_id,
                    ...
                    Err(why) => {
                        ...
                        let _ =
                            writeln!(&mut stderr, "parallel: command error: {}: {}", input, why);
                        let message = format!("{}: {}: {}\n", job_id, input, why);

pub mod disk {
    ...
    /// When using grouped mode, the `State` will tell the program whether the
    /// program is still processing, or if it has completed.
    pub enum State {
        /// The integer supplied with this signal tells the program which
    ...
    /// Sends messages received by a `Child` process's standard output and error
    /// and sends them to be handled by the grouped output channel.
    pub fn output(
        child: &mut Child,
        job_id: usize,
        name: String,
        ...
        if quiet {
            ...
            // Attempt to read from stdout and stderr simultaneously until both are
            // exhausted of messages.
            loop {

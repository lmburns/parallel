...
impl<IO: Read> DiskBufferReader<IO> {
    ...
    /// Takes an input argument that will optionally shift unused bytes at the
    /// end to the left, and then buffer into the adjacent bytes.
...
mod tests {
    ...
    fn test_disk_buffer_reader_simple() {
        ...
        let mut disk_buffer_reader = DiskBufferReader::new(
            Path::new("tests/buffer.dat"),
        ...
        let _ = disk_buffer_reader.buffer(0);
        assert_eq!(
            &file[BUFFER_SIZE..BUFFER_SIZE * 2],
            &disk_buffer_reader.data[..]
        );
        let _ = disk_buffer_reader.buffer(0);
    ...
    fn test_disk_buffer_reader_byte_shifting() {
        ...
        assert_eq!(
            &file[BUFFER_SIZE / 2 + 1..BUFFER_SIZE / 2 + 1 + BUFFER_SIZE],
            &disk_buffer_reader.data[0..BUFFER_SIZE]
        );
    }
}

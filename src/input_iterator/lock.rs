...
impl<IO: Read> InputsLock<IO> {
    /// Attempts to obtain the next input in the queue, returning `None` when it
    /// is finished. It works the same as the `Iterator` trait's `next()`
    /// method, only re-using the same input buffer.
    pub fn try_next(&mut self, input: &mut String) -> Option<usize> {
        ...
        if self.memory > 0 {
            if let Ok(mut mem_available) =
                sys_info::mem_info().map(|mem_info| mem_info.avail * 1000)
            {
                while mem_available < self.memory {
                    thread::sleep(Duration::from_millis(100));
                    if let Ok(mem_info) = sys_info::mem_info() {
                        mem_available = mem_info.avail * 1000;
                    }
                }
            }
        }

...
/// A token is a placeholder for the operation to be performed on the input
/// value.
pub enum Token {
    /// An argument is simply a collection of characters that are not
    /// placeholders.
    Argument(Cow<'static, str>),
    /// Takes the basename (file name) of the input with the extension removed.
    BaseAndExt,
    /// Takes the basename (file name) of the input with a custom suffix
    /// removed.
    BaseAndSuffix(&'static str),
    /// Takes the basename (file name) of the input with the directory path
    /// removed.
    Basename,
...
pub fn tokenize(
    ...
    for (id, character) in template.bytes().enumerate() {
        match (character, pattern_matching) {
            ...
            (b'{', false) => {
                ...
                // this will append the argument to the token list and disable argument
                // matching.
            ...
            (b'}', true) => {
                ...
                if id == pattern_start + 1 {
                    ...
                    match match_token(&template[pattern_start + 1..id], path, nargs)? {
                        ...
                        None => {
                            tokens.push(Token::Argument(Cow::Borrowed(
                                &template[pattern_start..id + 1],
                            )));
                        },
                    }
                }
    ...
    // In the event that there is leftover data that was not matched, this will add
    // the final string to the token list.
    if pattern_matching {
    ...
    match pattern {
        ...
        _ =>
            if pattern.starts_with('^') && pattern.len() > 1 {
                ...
                let ndigits = pattern
                    .bytes()
                    .take_while(|&x| (x as char).is_numeric())
                    .count();
                ...
                if ndigits != 0 {
                    ...
                    if ndigits == nchars {
                        ...
                        let argument =
                            Number::new(number, Token::Placeholder).into_argument(path)?;
...
//     assert_eq!(tokens, vec![Token::Argument("foo ".to_owned()),
// Token::Placeholder,         Token::Argument(" bar".to_owned())]);
// }

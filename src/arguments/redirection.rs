...
#[cfg(not(unix))]
/// At this time, only operating systems that feature a `proc` filesystem are
/// supported
...
#[cfg(unix)]
/// On UNIX systems that feature a `proc` filesystem, if `/proc/self/fd/0`
/// points to a location other than `/dev/pts` or `pipe:`, then the standard

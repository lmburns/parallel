...
/// Receives an input that is either an integer, or percent. If the string ends
/// with `%`, it will be calculated as a percent of the total number of CPU
/// cores. Otherwise, the number provided will be considered the number of jobs
/// to run in parallel.
pub fn parse(value: &str) -> Result<usize, ParseErr> {
    match (value.bytes().next().unwrap(), value.bytes().last().unwrap()) {
        (b'+', b'%') => value[1..value.bytes().count() - 1]
            .parse::<usize>()
            .map(|percent| {
        ...
        (b'-', b'%') => value[1..value.bytes().count() - 1]
            ...
            .map_err(|_| ParseErr::JobsNaN(value.to_owned())),
        (_, b'%') => value[0..value.bytes().count() - 1]
            .parse::<usize>()
            .map(|percent| (num_cpus::get() * percent) / 100)
            .map_err(|_| ParseErr::JobsNaN(value.to_owned())),
        (b'+', _) => value[1..value.bytes().count()]
            .parse::<usize>()

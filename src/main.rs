...
/// The command string needs to be available in memory for the entirety of the
/// application, so this is achievable by transmuting the lifetime of the
/// reference into a static lifetime. To guarantee that this is perfectly safe,
/// and that the reference will live outside the scope, the value will
/// also be leaked so that it is forced to remain in memory for the remainder of
/// the application.
...
/// The tokens will live throughout the entirety of the application, so it's OK
/// to mark it with a static lifetime. Prevents needing to copy the token vector
/// to each thread.
...
fn main() {
    ...
    // On Linux systems, check if transparent_hugepages is set to always and issue a
    // warning if true.
    if cfg!(target_os = "linux") {
        if let Ok(mut file) = File::open("/sys/kernel/mm/transparent_hugepage/enabled") {
            let mut buffer: [u8; 2] = [0, 0];
            if file.read_exact(&mut buffer).is_ok() && &buffer == b"[a" {
                let _ = writeln!(
                    stderr.lock(),
                    "ion: /sys/kernel/mm/transparent_hugepage/enabled is set to always instead of \
                     madvise. This will gravely effect the performance of Parallel."
                );
            }
        }
    }
    ...
    // Construct the paths of each of the required files using the base tempdir
    // path. These paths will be shared all through the application to avoid
    // needing to copy.
    ...
    // This file is required by the upcoming `InputIterator`. It will remain open
    ...
    let file = match fs::OpenOptions::new().read(true).open(&unprocessed_path) {
        ...
        Err(why) => {
            ...
            let _ = writeln!(
                stderr,
                "parallel: unable to open unprocessed file: {:?}: {}",
                &unprocessed_path, why
            );
            exit(1);
        },
    };

    // Initialize the `InputIterator` structure, which efficiently generates inputs
    // from the above `unprocessed` file until all arguments have been
    // processed, denoted by `args.ninputs`.
    let inputs = InputIterator::new(&unprocessed_path, file, args.ninputs)
        .expect("unable to initialize the InputIterator structure");

    // Coerce the `comm` `String` into a `&'static str` so that it may be shared by
    // all threads. This is safe because the original `comm` may no longer be
    // modified due to shadowing rules. It is also safe because `comm` lives to
    // the end of the program.
    let static_comm = unsafe { leak_string(comm) };

    // Attempt to tokenize the command argument into simple primitive placeholders.
    if let Err(error) = tokenize(
        &mut args.arguments,
        static_comm,
        &unprocessed_path,
        args.ninputs,
    ...
    // Coerce the arguments into a `&'static [Token]` to eliminate the need to make
    // a copy in each thread.
    let arguments = unsafe { static_arg(&args.arguments) };

    // If the `--dry-run` parameter was passsed, the program will simply print all
    // commands to execute and will subsequently quit. Otherwise, real work will
    // be performed.
    if args.flags & arguments::DRY_RUN != 0 {
        execute::dry_run(args.flags, inputs, arguments);
    } else {
        // The `InputIterator` will be wrapped within a `Mutex` so that it can safely be
        // shared across all of the upcoming threads. A `Mutex` is required
        // because each time a thread pulls the next input from the queue, it
        // needs to update various data fields which rely on threads waiting for
        // their turn to report back.
        let shared_input = Arc::new(Mutex::new(inputs));

        // This channel is used exclusively for signaling back to the main thread when a
        // task has completed or has errored.
        let (output_tx, input_rx) = channel::<State>();

        // Will contain handles to the upcoming threads to know when the threads are
        // finished.
        ...
        if args.flags & arguments::INPUTS_ARE_COMMANDS != 0 {
            ...
            for _ in 0..args.ncores {
                ...
                // Add the thread handle to the `threads` vector to know when to quit the
                // program.
                threads.push(handle);
            }
        } else {
            ...
            for slot in 1..args.ncores + 1 {
                ...
                // The command will be built from the arguments, and inputs will be transferred
                // to the command.
